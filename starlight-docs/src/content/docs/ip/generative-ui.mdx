...
```mermaid
flowchart TB
    Q["User Question\n'Show me top products by revenue'"] --> UNDERSTAND["1. UNDERSTAND\nParse intent & entities"]

    UNDERSTAND --> QUERY["2. QUERY\nGenerate & execute SQL"]

    QUERY --> ANALYZE["3. ANALYZE\nProfile result data"]

    ANALYZE --> SELECT["4. SELECT\nChoose optimal component"]

    SELECT --> CONFIGURE["5. CONFIGURE\nSet visualization parameters"]

    CONFIGURE --> RENDER["6. RENDER\nInteractive UI component"]
...
```mermaid
flowchart LR
    DATA["Query Results"] --> A1["Cardinality\nHow many unique values?"]
    DATA --> A2["Data Types\nNumeric? Categorical? Temporal?"]
    DATA --> A3["Distribution\nEven? Skewed? Outliers?"]
    DATA --> A4["Relationships\nPart-to-whole? Time series?"]
