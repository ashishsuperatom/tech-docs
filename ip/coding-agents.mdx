...
```mermaid
flowchart TB
    Q["Complex Question:<br>'Why did margins drop in the<br>Northeast last quarter?'"] --> AGENT["Superatom<br>Coding Agent"]

    subgraph CONTEXT["CONTEXT LAYER"]
        SM["Semantic Model<br>Data relationships"]
        TK["Tribal Knowledge<br>Org-specific rules"]
        HIST["Query History<br>Prior analysis"]
    end
    ...
    EXECUTION --> RESULT["Root Cause Analysis:<br>Product mix shift, Supplier costs,<br>Promotional discounting"]
...
flowchart LR
    subgraph LOOP["AGENT REASONING LOOP"]
        THINK["Think:<br>What do I need to know?"]
        ACT["Act:<br>Execute query/analysis"]
        OBSERVE["Observe:<br>Interpret results"]
        DECIDE["Decide:<br>Complete or continue?"]
    end
...
flowchart LR
    ...
    STRATEGY --> BEST["BEST<br>Highest quality<br>(Claude)"]
    STRATEGY --> FAST["FAST<br>Lowest latency<br>(Groq)"]
    STRATEGY --> BALANCED["BALANCED<br>Cost-effective<br>(GPT)"]
