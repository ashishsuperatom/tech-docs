...
The entire platform, including the orchestrator, data-source agents, WebSocket server, UI, reporting engine, and audit storage, runs on the customer's infrastructure. There is no cloud-hosted instance.
...
| **Role Mapping** | Groups and roles from the IdP map directly to Superatom permission levels, with no manual role configuration needed |
